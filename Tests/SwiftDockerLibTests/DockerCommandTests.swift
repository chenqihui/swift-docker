...
class DockerImageTests: XCTestCase {
    func testInitailizedWithVersion() throws {
        let version = "4.1"
        guard let destination = DockerImage(version: version, image: nil) else { throw TestError.failed }
        guard case let .swiftVersion(finalVersion) = destination else { throw TestError.failed }
        XCTAssertEqual(finalVersion, "4.1")
    }

    func testInitailizedWithImage() throws {
        let image = "swift:4.1"
        guard let destination = DockerImage(version: nil, image: image) else { throw TestError.failed }
    ...
    func testInitailizedWithEmptyImage() throws {
        let image = ""
        let version = "4.0"
        guard let destination = DockerImage(version: version, image: image) else { throw TestError.failed }
        guard case let .swiftVersion(finalVersion) = destination else { throw TestError.failed }
